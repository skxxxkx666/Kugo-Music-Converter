...
const (
	...
	ErrQMCContainerInvalid = "ERR_QMC_CONTAINER_INVALID"
	ErrQMCSTagUnsupported  = "ERR_QMC_STAG_UNSUPPORTED"
	ErrQMCKeyUnavailable   = "ERR_QMC_KEY_UNAVAILABLE"
	ErrQMCLoginRequired    = "ERR_QMC_LOGIN_REQUIRED"
	ErrQMCSessionExpired   = "ERR_QMC_SESSION_EXPIRED"
	ErrQMCEntitlement      = "ERR_QMC_ENTITLEMENT"
	ErrQMCNetwork          = "ERR_QMC_NETWORK"
...
var errorCatalog = map[string]errorMeta{
	...
	ErrQMCContainerInvalid: {userMessage: "QQ 音乐文件尾部无效。", suggestion: "请在 QQ 音乐客户端重新下载完整文件后重试。", severity: "error"},
	ErrQMCSTagUnsupported:  {userMessage: "暂不支持此类 STag QQ 音乐文件。", suggestion: "请使用当前 QQ 音乐 PC 客户端重新下载为 MFLAC/MGG musicex 文件。", severity: "warning"},
	ErrQMCKeyUnavailable:   {userMessage: "无法取得新版 QQ 音乐文件的解密密钥。", suggestion: "请确认 QQ 音乐客户端和账号状态正常后重试。", severity: "error"},
	ErrQMCLoginRequired:    {userMessage: "未找到可用的 QQ 音乐登录会话。", suggestion: "请启动 QQ 音乐 PC 客户端、登录下载该文件的账号后重试。", severity: "warning"},
	ErrQMCSessionExpired:   {userMessage: "QQ 音乐登录会话已过期。", suggestion: "请在 QQ 音乐客户端重新登录后重试。", severity: "warning"},
	ErrQMCEntitlement:      {userMessage: "当前 QQ 音乐账号无法取得此文件的解密密钥。", suggestion: "请登录下载该文件且仍具备对应资源权限的账号。", severity: "warning"},
	ErrQMCNetwork:          {userMessage: "无法连接 QQ 音乐取钥端点。", suggestion: "新版 MFLAC/MGG 每个转换批次都需要联网取钥，请检查网络后重试。", severity: "error"},
	...
	ErrUnsupportedFormat:   {userMessage: "不支持的输入文件格式。", suggestion: "支持 KGG/KGM/KGMA/VPR/NCM/KWM、MFLAC/MGG 及传统 QMC 格式。", severity: "warning"},
